...
const getAllStudentSemesterPaymentController = catchAsync(
  async (req: Request, res: Response) => {
    const filters = pick(req.query, studentSemesterPaymentFilterableFields);
    const options = pick(req.query, ['limit', 'page', 'sortBy', 'sortOrder']);
    const result =
      await StudentSemesterPaymentService.getAllStudentSemesterPayment(
        filters,
        options
      );
    sendResponse(res, {
...
export const StudentSemesterPaymentController = {
  getAllStudentSemesterPaymentController,
};
